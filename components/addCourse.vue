      ...
      <v-row>
        <v-col class="text-center">
          <v-card>
            ...
            <v-container v-for="(item, i) in tmpLessons" :key="i">
              <input type="file" :id="i" style="display: none" @change="handleFileObject($event, i)"/>
              <input
                type="button"
                value="Browse..."
                @click="test(i)"
              />
              <p>{{tmpLessons[i].video.name}}</p>
              <!-- <input
                type="file"
                id="customFile"
                @change="handleFileObject($event, i)"
              /> -->
              <v-btn @click="decreaseQuesstion(i)"> decrease</v-btn>

              <v-card-text class="row ma-0">
                <v-text-field
                  v-model="tmpLessons[i].title"
                  class="col col-12 col-sm-6"
                  name="point"
                  label="Title"
                  hint="please fill title"
                ></v-text-field>

                <v-textarea
                  v-model="tmpLessons[i].summary"
                  outlined
                  class="col col-12"
                  label="Summary"
                  hint="Description"
